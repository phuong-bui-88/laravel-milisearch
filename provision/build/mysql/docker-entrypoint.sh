#!/bin/bash
# Origin file: https://github.com/docker-library/mysql/blob/master/5.7/docker-entrypoint.sh
...
docker_setup_db() {
	...
	if [ -n "$MYSQL_USER" ] && [ -n "$MYSQL_PASSWORD" ]; then
		...
		if [ -n "$MYSQL_DATABASE_2" ]; then
			...
			docker_process_sql --database=mysql <<<"GRANT ALL ON \`$MYSQL_DATABASE_2\`.* TO '$MYSQL_USER'@'%' ;"
		fi
		# End_Add
